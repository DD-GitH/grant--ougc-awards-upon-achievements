...
if($mybb->input['action'] == 'add')
{
    ...
    $form_container->output_row("Achievement Type", "Achievement type when granting an award", $form->generate_select_box("actype", array('posts' => 'Posts Count', 'onlinetime' => 'Online Time', 'reputation' => 'Reputation', 'newpoints' => 'Points', 'shouts' => 'Shouts'), $selected=array(), $options=array()));
    ...
    $form_container->output_row("FIDs", "If posts, write FIDs of the sections separated for comma, let empty for all forums.", $form->generate_text_box('fids', $mybb->input['fids'], array('id' => 'fids')), 'fids');
...
if($mybb->input['action'] == 'edit')
{
	...
	if($mybb->request_method == "post")
	{
        $did = $db->query("UPDATE ".TABLE_PREFIX."awardgranting SET awardid='{$mybb->input['awardid']}', actype='{$mybb->input['actype']}', acvalue='{$mybb->input['acvalue']}', fids='{$mybb->input['fids']}' WHERE id='{$task_id}'");
        if($did)
        {
